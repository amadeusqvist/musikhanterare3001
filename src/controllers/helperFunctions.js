...
var playlistsDbPath = path.join(dataFolderPath, 'playlistsdb.json');
var songsDbPath = path.join(dataFolderPath, 'songsdb.json');
function loadSongs() {
    try {
        var data = fs.readFileSync(songsDbPath, 'utf8');
...
function loadPlaylists() {
    try {
        var data = fs.readFileSync(playlistsDbPath, 'utf8');
        return JSON.parse(data);
    }
    catch (error) {
